...
class Tesla(Cable):
    ...
    p2_fifth_row  = [f"A{i}P2" for i in range(87, 0, -1)]
    p2_fourth_row = [f"B{i}P2" for i in range(87, 0, -1)]
    p2_third_row  = [f"C{i}P2" for i in range(87, 0, -1)]
    p2_second_row = [f"D{i}P2" for i in range(87, 0, -1)]
    p2_first_row  = [f"E{i}P2" for i in range(87, 0, -1)]

    p2_order = p2_first_row + p2_second_row + p2_third_row + p2_fourth_row + p2_fifth_row
    p1_order = p1_first_row + p1_second_row + p1_third_row + p1_fourth_row + p1_fifth_row
    ...
    # Updated: allow multiple letters before digits in parens (R52, FS1, etc.)
    CODE_DASH_ANN_WITH_PAREN_RE = re.compile(
    ...
    def extract_channel(self, text: str) -> Tuple[str, ...]:
        """
        Extract concatenated codes like 'A34DIB' and 'E54P2' from inputs such as:
        '..., A34-DIB  (R52), E54-P2  (R52), ...'
        '..., E1-DIB  (FS1), A1-P1  (FS1), ...'

        Returns exactly the concatenations (LEFT+RIGHT), in order of appearance.
        """
        if not isinstance(text, str) or not text:
            return ()

        # Normalize common hyphen/space variants seen in copied text
        cleaned = (
            text.replace("\u00A0", " ")
                .replace("\u2011", "-")
                .replace("\u2013", "-")
        ...
        out: List[str] = []
        for m in self.CODE_DASH_ANN_WITH_PAREN_RE.finditer(cleaned):
            left = (m.group("left") or "").upper()    # e.g., A34 / E54
    ...
    def _slice_row(labels, values, row_letter):
        ...
        for lab, val in zip(labels, values):
            if lab.startswith(row_letter):
                row_vals.append(val)
                row_nums.append(Tesla._extract_num(lab))
    ...
    def _grid_from(values, order, row_letters):
        ...
        for letter in row_letters:
            row_vals, row_nums = Tesla._slice_row(labels, values, letter)
    ...
    def make_analog_heatmap(
        ...
        *,
        ...
        dib_grid = np.array(dib_grid, dtype=float, copy=True)  # safe copy
        # Set B = A, G = F
        dib_grid[1, :] = dib_grid[0, :]  # B ← A
        dib_grid[6, :] = dib_grid[5, :]  # G ← F
        # (Plotly accepts numpy arrays, so leaving dib_grid as np.array is fine.)

        # P1: rows A..E, columns 1..87
        p1_row_letters = list("ABCDE")
        p1_grid, p1_xticks = self._grid_from(p1_vals, self.p1_order, p1_row_letters)
        p1_grid = np.array(p1_grid, dtype=float, copy=True)  # safe copy
        p1_grid[0:4, :] = p1_grid[0, :]   # rows 0,1,2,3 <- row 0
        ...
        # dib (middle) — uses imputed dib_grid
        fig.add_trace(
            go.Heatmap(
                z=dib_grid,
                x=dib_xticks,
        ...
        arrow_end = max(dib_xticks)   # or dib_xticks[-1]
        arrow_start = dib_xticks[-15]
        ...
        fig.update_yaxes(autorange="reversed", row=1, col=1)  # P2
        fig.update_xaxes(autorange="reversed", row=1, col=1)
        fig.update_yaxes(autorange="reversed", row=2, col=1)  # DIB
        fig.update_yaxes(autorange="reversed", row=3, col=1)  # P1


        # Hide x-axis ticks on all rows (as in your original)
        for r in (1, 2, 3):
            fig.update_xaxes(showticklabels=False, ticks="", showgrid=False, row=r, col=1)
