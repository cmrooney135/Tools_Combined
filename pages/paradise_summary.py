...
# --------------------------
CABLE_FAMILY = "paradise"  # <---- Set to "paradise" in this version
...
def _hist(
    ...
    if overflow is not None and not np.isnan(overflow):
        fig.update_xaxes(range=[0.0, float(overflow)])
    else:
        xmax = float(np.nanmax(clipped)) if clipped.size else 1.0
        fig.update_xaxes(range=[0.0, xmax])
    if log_x:
